...
pub const JIT = struct {
        ...
        const lowered = switch (method) {
            ...
            .bz_setTryCtx => l.functionType(
                ...
                &[_]*l.Type{
                    ...
                    self.context.getContext().pointerType(0),
                    // ptr to jmp_buf
                    (try self.lowerExternApi(.jmp_buf)).pointerType(0),
                },
                2,
                .False,
            ),
            ...
            .jmp_buf => self.context.getContext().intType(@sizeOf(c_int)).arrayType(
                // That size is platform specific hence the cImport
                @sizeOf(jmp.jmp_buf) / @sizeOf(c_int),
            ),
        };
    ...
    fn generateTry(self: *Self, try_node: *TryNode) VM.Error!?*l.Value {
        ...
        // Allocate a jmp_buf
        const env = self.state.builder.buildAlloca(
            try self.lowerExternApi(.jmp_buf),
            "env",
        );
